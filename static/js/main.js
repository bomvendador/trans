;(function () {
	...
	var whyusAnimate = function() {
		var whyus = $('#fh5co-whyus');
		if ( whyus.length > 0 ) {
			...
			whyus.waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {
					...
					var sec = whyus.find('.to-animate').length,
						sec = parseInt((sec * 200) + 400);

					setTimeout(function() {
						whyus.find('.to-animate').each(function( k ) {
							var el = $(this);
					...
					setTimeout(function() {
						whyus.find('.to-animate-2').each(function( k ) {
							var el = $(this);
